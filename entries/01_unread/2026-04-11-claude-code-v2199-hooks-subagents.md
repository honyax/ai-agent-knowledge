...
CLAUDE.md でのフック設計や自動化ワークフローに直接影響する変更。ヘッドレス実行中に人間の承認を挟みたいケース（例: 本番デプロイ前の確認）が `defer` で実現可能になる。MCP の5秒タイムアウトは安定性向上につながり、複数MCPサーバー利用時に体感できる。
...
- `--mcp-config` で起動時に最大5秒でサーバー接続を打ち切る（遅いサーバーがある環境でのスタートアップ改善）
