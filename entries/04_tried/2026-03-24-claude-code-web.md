...
**実践後の考察**
Web を経由することでデスクトップPCと外出先のノートPCやスマホとの連携が実現し、Web上で自律的に作業を進めることができる。デバイスをまたいだシームレスな開発フローとして非常に有用性を感じた。

何か思いついた時にスマホで作業を開始・進行できるのは本当に便利。移動中や外出先でも開発タスクを投入できるワークフローは今後意識的に活用していきたい。

セッションの移行に関しては、ローカル→リモートへの転送ができないため片手落ち。実運用ではセッション移行にこだわらず、ブランチへのコミットで作業を継続する方が現実的と判断した。
