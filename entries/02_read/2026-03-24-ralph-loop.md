...
参考にはなるが、ゲーム開発においては現状あまり積極的に使うイメージは湧かない。
ゲームは動作確認が不可欠で、AI自身がゲームを実際に動かして挙動を検証する手段がない。
長期ループで大量に実装させても、途中の動作確認を人間がはさまないと、仕様のズレや不具合が積み重なって後から修正コストが大きくなりやすい。

一方、要件が事前に明確に固まっており、動作確認が自動テストで完結するようなプロダクト（Webサービスや CLIツール等）であれば、このアプローチは有効だと思う。
ゲーム開発に応用するとすれば、ユニットテストで完結するロジック層（データ処理・計算式など）や、UIレイアウトのような視覚確認不要な部分に限定するのが現実的か。
