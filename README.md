...
| `/try entries/2026-03-24.md` | エントリの内容を実践・実験 |
| `/review entries/2026-03-24.md` | 実践結果から感想・考察を生成 |
| `/review week` | 週次レビューを生成 |
| `/status` | ナレッジベースの状態を確認 |
...
1. /status          → 状態確認
2. /catch-up        → 情報収集（自動）
3. 気になるものを /try → 実践（半自動）
4. /review week     → 週次レビュー（自動）
5. git add -A && git commit → 保存
```
