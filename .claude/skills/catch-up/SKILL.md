...
1. **前回実行日の取得**: `entries/` 内の最新ファイルの日付を前回実行日として取得する
2. **重複チェック用データ収集**: 既存エントリの `source_urls` とタイトルを収集しておく
...
`entries/` ディレクトリのファイル名を確認し、最新の日付（YYYY-MM-DD形式）を取得する。
...
`entries/` が空の場合は「直近1週間」をデフォルトとする。
...
`templates/entry-template.md` を読み、そのフォーマットに従って `entries/YYYY-MM-DD-slug.md` を生成する。
同日のファイルが既に存在する場合は `entries/YYYY-MM-DD-slug-2.md` のように連番をつける。
