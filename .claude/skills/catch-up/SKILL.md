...
2. **重複チェック用データ収集**: 全サブフォルダの既存エントリのうち **直近30日分** の `source_urls`・タイトル・3行要約を収集しておく
...
エントリ生成前に以下を確認し、該当する記事はスキップする。比較対象は **直近30日分のエントリのみ**（古いアーカイブは対象外）。

1. **URL重複**: 記事の URL が既存エントリのいずれかの `source_urls` に含まれている
2. **タイトル類似**: 記事のタイトルと既存エントリのタイトルが本質的に同じ内容を指している
3. **トピック重複**: 同じ出来事・リリース・発表を別メディアが報じているだけの記事はスキップする
   - 判定方法: 収集済みの3行要約と照合し、製品・バージョン・機能・発表の主体が一致する場合はトピック重複とみなす
   - 例: 「Claude 4 リリース」を報じたエントリが既にある場合、別メディアの同報道はスキップ
   - 例外: 新たな技術的詳細・異なる観点（セキュリティ評価・パフォーマンス比較等）を含む場合は別エントリとして生成する

重複と判断した場合は生成をスキップし、スキップした旨（どのルールに該当したか）を出力後のサマリーに記載する。
